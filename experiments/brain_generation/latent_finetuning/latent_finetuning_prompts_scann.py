...
from dataset.build_dataset import MRIDatasetScanners
...
class LatentFineTuning:
    ...
    def _setup_dataset(self):
        ...
        dataset = MRIDatasetScanners(data_dir, transform=preprocess, latents=True, prompt=True)
    ...
    def _log_prompt_similarity(self, step=None):
        ...
        sim_philips_siemens = F.cosine_similarity(embeddings[0], embeddings[1], dim=-1).item()
        sim_philips_ge = F.cosine_similarity(embeddings[0], embeddings[2], dim=-1).item()
        sim_siemens_ge = F.cosine_similarity(embeddings[1], embeddings[2], dim=-1).item()

        if self.config['logging']['logger_name'] == 'wandb':
            self.accelerator.get_tracker("wandb").log({
                "cos_sim/phillips-siemens": sim_philips_siemens,
                "cos_sim/phillips-ge": sim_philips_ge,
                "cos_sim/siemens-ge": sim_siemens_ge,
            }, step=step if step is not None else self.global_step)
        else:
            self.accelerator.print(f"[Step {self.global_step}] Cosine Similarities:")
            self.accelerator.print(f"  Philips - Siemens: {sim_philips_siemens:.4f}")
            self.accelerator.print(f"  Philips - GE: {sim_philips_ge:.4f}")
            self.accelerator.print(f"  Siemens - GE: {sim_siemens_ge:.4f}")
    ...
    def train(self):
        ...
        # Check if the 'Philips', 'Siemens', and 'GE' tokens are trainable
        start_time = time.time()
        logger.info("Starting training...")
        embedding_layer = self.ldm.text_encoder.get_input_embeddings()
        token_ids = [self.ldm.tokenizer.convert_tokens_to_ids(t) for t in ["Philips", "Siemens", "GE"]]
        for token, tid in zip(["Philips", "Siemens", "GE"], token_ids):
...
if __name__ == "__main__":
    config_path = "config_latent_finetuning_scanners.yaml"
