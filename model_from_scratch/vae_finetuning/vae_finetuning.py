...
def main():
    ...
    lr_scheduler = get_scheduler(
        ...
        num_training_steps= max_train_steps #(len(train_dataloader) * num_epochs), #* config['training']['gradient_accumulation']['steps']?
    )
    ...
    for epoch in range(num_epochs): # Loop over the epochs
        ...
        for _, batch in enumerate(train_dataloader): # Loop over the batches
            with accelerator.accumulate(model): # moved to the beginning of the loop
                ...
                noise = torch.randn_like(latents) # Sample noise to add to the images and also send it to device (2nd thing in device)
